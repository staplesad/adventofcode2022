...
int find_min_suffice(struct node n, int bar, int current_min) {
  int tmp;
  if (n.is_dir) {
    if (n.size >=bar) {
      printf("%s: %d\nmin: %d\n\n", n.dir_name, n.size, current_min);
      if (n.size < current_min){
        current_min = n.size;
      }
      for (int i=0; i<n.num_children; i++){
        printf("child %d\n", i);
        printf("is dir: %d\n", n.children[i].is_dir);
        tmp = find_min_suffice(n.children[i], bar, current_min);
        printf("tmp: %d\n");
        if (tmp < current_min){
          current_min =  tmp;
        }
...
int main(void) {
  ...
  int current_size = 70000000 - total_size;
  int needed_size = 30000000 - current_size;
  int ans = find_min_suffice(tree, needed_size, 70000000);
  printf("total: %d\n", total_size);
  printf("%d\n", needed_size);
  printf("%d\n", ans);
  //print_tree(tree);
